...
class BookingPropertiesCrawler(scrapy.Spider):
    ...
    async def parse_search_results(self, response):
        ...
        try:
            ...
            for hotel in hotel_cards:
                ...
                if len(self.pages) >= self.max_results:
    ...
    async def scroll_and_load_more(self, page, response):
        # Scroll to the bottom of the page to trigger the "Load more results" button to load
        await page.evaluate("""
            window.scrollTo(0, document.body.scrollHeight);
        """)

        # Wait for the "Load more results" button to appear
        load_more_button_selector = 'button span.e4adce92df'
        try:
            ...
            if load_more_button:
                self.log("Clicking 'Load more results' button.", logging.INFO)
                await load_more_button.click()
                
                # Recursively call to load more results
                await self.parse_search_results(response)

        except Exception as e:
            self.log(f"Error while trying to load more results: {e}", logging.WARNING)
